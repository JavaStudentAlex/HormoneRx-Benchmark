...
```bash
npm install          # one install for frontend AND backend (single language)

npm run backend      # realtime engine -> http://localhost:8000  (terminal 1)
...
TypeScript end to end. Frontend: React + Vite + Tailwind (hand-built shadcn-style primitives). Backend: Node 20+ + Express + ws, run with tsx. Tests: Vitest (frontend and backend) · Playwright for end-to-end checks and screenshots.

> History: v0.2.x implemented the same engine in Python (FastAPI); v0.3.0 ported it 1:1 to TypeScript so the whole project is one language. The wire format, tests, and benchmark gold labels are unchanged — the port reproduces 96/96 tests and identical benchmark metrics.
